...
- 폴더별 기본값: 모드(분담 / Claude만 / GLM만), 메인 모델·effort, 작업자 모델·effort, 열기 방식(런처 안 / 터미널 탭 / 새 창 / VS Code)
- 런처 안 터미널: 런처 창의 탭에서 바로 Claude Code를 쓰고, 여러 폴더를 탭으로 오가며 작업
- 여러 폴더를 골라 Windows Terminal 한 창에 분할로 열기 (최대 2×2, 그 이상은 같은 창의 새 탭)
...
- 행의 **열기** 또는 행 더블클릭으로 엽니다. 열기 방식이 **런처 안**이면 런처 창 위쪽에 탭이 생기고 그 안에서 바로 작업합니다. 체크한 뒤 **선택한 폴더를 분할 창으로 열기**를 누르면 Windows Terminal 한 창에 나란히 열립니다.
- 런처 안 터미널에서는 Ctrl+Tab으로 탭을 바꿉니다. 글자를 선택한 뒤 Ctrl+C나 우클릭으로 복사하고, Ctrl+V나 우클릭으로 붙여넣습니다. Shift+Enter는 Claude 입력창의 줄바꿈입니다.
- 런처 창을 닫으면 그 안의 세션도 끝나므로 닫기 전에 확인 창이 뜹니다. 업데이트를 적용할 때도 같습니다.
- 모든 폴더의 열기 방식을 한 번에 바꾸려면 설정 → 새 폴더 기본값에서 열기 방식을 고르고 **이 열기 방식을 모든 폴더에 적용**을 누릅니다.
- 행 왼쪽 색은 모드 색이고, 열린 탭(런처 안, Windows Terminal)도 같은 색입니다.
