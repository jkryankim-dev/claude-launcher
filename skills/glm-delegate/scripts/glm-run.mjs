...
const CFG = {
  ...
  effort: (E.GLM_EFFORT || 'max').toLowerCase(),
  maxTurns: posInt(E.GLM_MAX_TURNS, 200),
  timeoutMin: posNum(E.GLM_TIMEOUT_MIN, 60),
  heartbeatSec: posInt(E.GLM_HEARTBEAT_SEC, 120),
  reportLines: posInt(E.GLM_REPORT_LINES, 80),
...
const RULES_EDIT = n => `[진행 절차 — 순서대로 모두 수행한다]
1. 파악: 고치기 전에 대상 파일을 전부 끝까지 읽는다. 대상이 import하거나 대상을 import하는 파일, 관련 테스트, 타입 정의, 프로젝트 규칙(CLAUDE.md, lint·tsconfig, 비슷한 기존 코드)도 읽어 관례를 익힌다. 대상 목록을 grep으로 다시 뽑아 명세와 대조하고, 파일마다 무엇을 어떻게 바꿀지 체크리스트를 만든 뒤에 수정을 시작한다.
2. 구현: 명세의 규칙과 예시를 모든 대상에 빠짐없이 적용한다. 각 변경마다 빈 값·null·에러 경로·경계값·동시성·인코딩처럼 해당되는 엣지 케이스를 따져 처리한다. 기존 코드 스타일·네이밍·타입 엄격도를 그대로 따르고, 임시 코드(TODO·console.log·주석 처리)를 남기지 않는다.
3. 테스트: 동작이 바뀌는 변경마다 프로젝트의 테스트 프레임워크로 테스트를 추가·확장한다(정상·경계·실패 케이스). 테스트 파일이 수정 허용 범위 안이면 직접 쓰고, 밖이면 필요한 테스트 내용을 "범위 밖 필요 변경"에 구체적으로 적는다.
4. 검증: 검증 명령과 허용된 테스트·타입 검사·린트를 모두 실행한다. 실패하면 원인을 고쳐 다시 돌린다(최대 5번). 그래도 실패하면 멈추고 무엇이 왜 안 되는지 보고한다.
5. 자체 리뷰: 끝났다고 생각되면 git diff로 변경 전체를 처음부터 다시 읽는다. 놓친 대상, 규칙 불일치, 깨진 참조·import, 죽은 코드, 회귀 가능성을 찾아 고친 뒤 검증을 한 번 더 돌린다.

[제약]
6. [작업 명세]에 적힌 일과 그것을 올바르게 완성하는 데 필요한 일만 한다. 요청받지 않은 리팩터링·이름 변경·포맷팅은 하지 않는다.
7. 수정 허용 범위 안의 파일만 고친다. 범위 밖 수정이 꼭 필요하면 고치지 말고 "범위 밖 필요 변경"에 무엇을 왜 바꿔야 하는지 적는다.
8. 애매하면 기존 동작을 유지하는 쪽을 고르고, 근거와 대안을 "설계 판단"에 적는다. 질문하지 말고 끝까지 진행한다.
9. 파일은 필요한 부분만 Edit 도구로 고친다. Write(전체 쓰기)는 새 파일을 만들 때만 쓴다.
10. 비밀값(.env, 키, 토큰)은 읽거나 출력하지 않는다. git 커밋·브랜치 조작, 패키지 설치, 대량 삭제를 하지 않는다. 다른 에이전트에 다시 위임하지 않는다.
11. 셸 명령은 Bash 도구로 실행한다. 권한이 거부된 명령은 다시 시도하지 말고 보고서에 적는다.

[보고서]
12. 마지막 응답은 아래 형식의 보고서만 쓴다(${n}줄 이내, 해당 없으면 "없음"). 사실만 쓰고, 하지 않은 일을 했다고 쓰지 않는다:
## 결과
완료 | 부분 완료 | 실패 — 한두 줄 요약
## 변경 파일
- 경로 — 바꾼 내용 한 줄 (같은 변경끼리 묶음)
## 설계 판단
- 무엇을 어떻게 판단했고 왜인지, 검토한 대안
## 처리한 엣지 케이스
## 테스트
- 추가·수정한 테스트와 각각이 확인하는 것
## 범위 밖 필요 변경
## 검증
- 명령 → 통과/실패 (실패면 핵심 오류 한 줄)
## 리스크·후속 제안`;

const RULES_SCAN = n => `[진행 절차 — 순서대로 모두 수행한다]
1. 범위 안의 관련 파일을 전부 읽는다. 일부만 보고 추정하지 않는다. grep으로 후보를 뽑은 뒤 각 후보를 파일에서 직접 확인해 오탐을 걸러내고, 마지막에 grep 총수와 발견 수를 대조해 빠진 것이 없는지 확인한다.
2. 발견 사항마다 근거(\`경로:줄\`, 한 줄 인용), 왜 문제인지·어떤 영향이 있는지, 구체적인 수정 제안을 함께 적는다. 같은 패턴은 묶고 개수를 밝힌다.
3. 명세가 구조·흐름·의존 관계를 묻거나 그것을 알아야 답이 되면, 데이터 흐름·호출 관계·모듈 경계를 정리해 적는다.
4. 결론부터 쓴다. 대상 목록을 요청받으면 빠짐없이 한 줄에 하나씩 쓴다.

[제약]
5. 파일을 수정하지 않는다(읽기 전용 조사). 명세와 무관한 조사는 하지 않는다.
6. 비밀값은 출력하지 않는다. 다른 에이전트에 위임하지 않는다. 셸 명령은 Bash 도구로 실행하고, 거부된 명령은 다시 시도하지 않는다.

[보고서]
7. 마지막 응답은 아래 형식의 보고서만 쓴다(${n}줄 이내, 해당 없으면 "없음"):
## 요약
## 발견 사항
- \`경로:줄\` — 내용 · 영향 · 제안
## 구조·흐름
...
function buildPrompt(spec, cfg, ctx) {
  return [
    '너는 숙련된 코딩 작업자다. 설계자가 쓴 [작업 명세]를 완성도 높게 끝까지 수행한다. 대화 상대가 없으니 질문하지 말고 스스로 판단해 진행하고, 마지막 응답으로 보고서만 쓴다. 빨리 끝내는 것보다 빠짐없이·깊게·검증된 결과를 내는 것이 우선이다. 시간과 턴은 넉넉하니 아끼지 않는다.',
